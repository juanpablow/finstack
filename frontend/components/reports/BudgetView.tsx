...
export function BudgetView({
  ...
  const totalSpent = Object.values(categoryExpenses).reduce(
    (sum, expenses) =>
  ...
  const segments = categories.reduce<
    Array<{ color: string; offset: number; length: number; percentage: number }>
  >((acc, cat) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      {/* Orçamento Card */}
      <div className="flex items-center justify-center bg-white rounded-2xl shadow-md p-8">
        {/* Donut Chart */}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-3 text-sm font-bold text-gray-900">
                  Budget
                </th>
                <th className="text-left py-3 text-sm font-bold text-gray-900">
                  Valor gasto
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4 text-sm text-gray-700">
                      R$ {formatCurrencyBRL(budgetAmount)}
                    </td>
                    <td className="py-4 text-sm text-gray-700">
                      R$ {formatCurrencyBRL(spent)}
                    </td>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t-2 border-gray-300">
            <div>
              <p className="text-xl font-bold text-green-600">
                R$ {formatCurrencyBRL(totalSpent)}
              </p>
            ...
            <div>
              <p className="text-xl font-bold text-green-600">
                R$ {formatCurrencyBRL(totalToSpend)}
              </p>
              <p className="text-xs text-gray-600 font-semibold">
                TOTAL A GASTAR
              </p>
            ...
            <div>
              <p className="text-xl font-bold text-green-600">
                {totalToSpend > 0
                  ? formatPercentage((totalSpent / totalToSpend) * 100)
                  : "0.00%"}
              </p>
